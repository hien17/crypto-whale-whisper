...
const TraderCard: React.FC<TraderCardProps> = ({ trader }) => {
  ...
  return (
    <motion.div 
      className={`trader-card w-full transition-all duration-300 group ${trader.isWhale ? 'border-crypto-teal/20' : 'border-crypto-blue/20'}`}
      ...
      whileHover={{ 
        y: -3, 
        boxShadow: trader.isWhale ? '0 5px 20px rgba(45, 212, 191, 0.2)' : '0 5px 20px rgba(30, 174, 219, 0.2)' 
      }}
    >
      {/* High contrast decorative line */}
      <div className={`absolute top-0 left-0 w-full h-0.5 ${trader.isWhale ? 'bg-crypto-teal' : 'bg-crypto-blue'} opacity-60`}></div>
      ...
      <div className="flex justify-between items-start">
        <div className="flex items-center">
          <div 
            ...
            style={{ background: trader.avatar ? 'transparent' : generateAvatarGradient() }}
          >
            {trader.avatar ? (
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs text-gray-400 font-mono">active {Math.floor(Math.random() * 24)}h ago</span>
          </div>
        </div>
        <div className="rounded-full bg-blue-500/10 p-1 group-hover:bg-blue-500/20 transition-colors">
          <div className={`w-6 h-6 rounded-full bg-gradient-to-r ${trader.isWhale ? 'from-crypto-teal to-crypto-green' : 'from-crypto-blue to-crypto-teal'} flex items-center justify-center`}>
        ...
        <div>
          ...
          <div className="text-lg font-bold text-white flex items-center">
            ...
            <div className="ml-1 h-3 w-[30px] bg-gray-800 rounded-full overflow-hidden">
              <div 
                className={`h-full ${trader.winRate > 0.6 ? 'bg-crypto-positive' : 'bg-crypto-blue'} rounded-full`}
